...
for video in data.data:
    ...
    for (i,j) in itertools.izip(frames,frames1):
        ...
        # print(i)
        # print(j)
        # features = model.extract(i)
        ifeatures = model.extract(j)
        cfeatures = np.loadtxt(i,dtype=float)
        features = np.concatenate((cfeatures, ifeatures), axis=None)
        #features=features.flatten('C')
        # print(features)
        # print(features.shape)
        # print('HellooooooooooooooooOOOOOOOOOOOOOOOOOWWWWWWWWWWWWWWWWOOOOOOIIIIIIIIIIIIII!!')
